/*
 * @Author: chenyusheng
 * @Date: 2018-01-10 19:05:09
 * @Last Modified by: mikey.zhaopeng
 * @Last Modified time: 2018-04-11 13:25:20
 * 辅助函数
 */
...
export function getLocal (key = STORAGE_USER_KEY) {
 ...
 // 获取
export function clearLocal (key = STORAGE_USER_KEY) {
  // console.log('get local operation')
...
// 增加时
export function AddHours (date, value) {
...
export function getUrlParam (name) {
  ...
  if (r != null) {
    return decodeURIComponent(r[2])// 这里为什么是从第三个字符解析呢？不知道这样理解对不对，因为路径后面的参数形式为参数名=参数值，而第一个字符为参数名，第二个为=，第三个就为参数值了。。。因为下面调用的时候得出的就是参数值
  }
}
